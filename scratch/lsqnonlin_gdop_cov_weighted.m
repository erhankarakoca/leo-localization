...
[~, Q] = calculateGDOP(selectedSatPositions, estUEPosTDOAError);
Q = Q + eye(size(Q)) * 1e-9; % Add small regularization term
% W = inv(Q(1:3, 1:3));
% W = diag(1 ./ diag(Q(1:3, 1:3))); % Use the inverse of the variances for (x, y, z)
W=1;
% Define the weighted objective function
ObjectiveEstimationErrorTDOAWeighted = @(p) arrayfun(@(k) ...
    sqrt((p(1:3) - selectedSatPositions(pairs(k, 1), :)) * W * ...
         (p(1:3) - selectedSatPositions(pairs(k, 1), :))') - ...
    sqrt((p(1:3) - selectedSatPositions(pairs(k, 2), :)) * W * ...
         (p(1:3) - selectedSatPositions(pairs(k, 2), :))') - ...
    radiiDifferenceswithError(k), 1:size(pairs, 1));
...
estUEPosTDOAError = lsqnonlin(ObjectiveEstimationErrorTDOAWeighted, ...
                               estUEPosTDOAError(1:3), ...
                               lowerBound, ...
                               upperBound, ...
                               options);
