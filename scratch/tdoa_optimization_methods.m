...
% Objective function for TDOA error
ObjectiveEstimationErrorTDOA_fminunc = @(p) ...
    sum(arrayfun(@(k) ...
        (sqrt((p(1) - selectedSatPositions(pairs(k, 1), 1))^2 + ...
              (p(2) - selectedSatPositions(pairs(k, 1), 2))^2 + ...
              (p(3) - selectedSatPositions(pairs(k, 1), 3))^2) - ...
         sqrt((p(1) - selectedSatPositions(pairs(k, 2), 1))^2 + ...
              (p(2) - selectedSatPositions(pairs(k, 2), 2))^2 + ...
              (p(3) - selectedSatPositions(pairs(k, 2), 3))^2) - ...
         radiiDifferenceswithError(k))^2, 1:size(pairs, 1)));

% Objective Function for PSO
objectiveFunction = @(p) sum(arrayfun(@(row) ...
    abs(sqrt((p(1) - selectedSatPositions(pairs(row, 1), 1))^2 + ...
             (p(2) - selectedSatPositions(pairs(row, 1), 2))^2 + ...
             (p(3) - selectedSatPositions(pairs(row, 1), 3))^2) - ...
        sqrt((p(1) - selectedSatPositions(pairs(row, 2), 1))^2 + ...
             (p(2) - selectedSatPositions(pairs(row, 2), 2))^2 + ...
             (p(3) - selectedSatPositions(pairs(row, 2), 3))^2) - ...
        radiiDifferencesWithError(row)), 1:size(pairs, 1)));

%% Set Eq. Solver
options = optimoptions('lsqnonlin', 'Display', 'iter');
options_fminunc = optimoptions('fminunc', 'Display', 'iter', 'Algorithm', 'quasi-newton');
...
estUEPosTDOAError_fminunc = fminunc(ObjectiveEstimationErrorTDOA_fminunc, ...
                                     initialGuess(1:3), ...
                                     options_fminunc);
...
localization_error_with_radii_error_fminunc = norm(estUEPosTDOAError_fminunc(1:3) - actualUEPosition);
...
disp('Localization Error with TDOA error ls (meters):');
disp(localization_error_with_radii_error);

disp('Localization Error with TDOA error fminunc (meters):');
disp(localization_error_with_radii_error_fminunc);
...
function tdoa = compute_tdoa_from_ue(ue_pos, sat_pos, c)
    % Computes TDOA values given UE position and satellite positions
    distances = vecnorm(sat_pos - ue_pos, 2, 2); % Distance from UE to satellites
    toa = distances / c; % Time of Arrival
    tdoa = toa - toa(1); % TDOA relative to Sat 1
    tdoa = tdoa(2:end); % Exclude reference Sat 1
end
